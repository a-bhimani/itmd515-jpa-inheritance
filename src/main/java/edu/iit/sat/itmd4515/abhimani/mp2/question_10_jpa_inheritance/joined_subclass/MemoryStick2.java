 ...
 */
@Entity
@Table(name="joined_sdcards")
@NamedQueries({
    @NamedQuery(name="MemoryStick2.retrieveAll", query="SELECT s FROM MemoryStick2 AS s")
})
public class MemoryStick2
    ...
    public MemoryStick2(String Model_No, int Capacity, ConnectivityInterface ci, int speed_class, float weight){
	...
	try{
	    return ("/StorageDevice2/MemoryStick2{Id:"+this.getStorageId()+", S/N:\""+this.getSerialNo()+"\", M/N:\""+this.getModelNo()+"\", Capacity:\""+this.getStorageCapacity()+"\", Interface:\""+this.getConnectivityInterface()+"\", Class:\""+this.getSpeed_class()+"\", Weight:\""+this.getWeight()+" gms\"}");
	}catch(Exception ex){
	    ex.printStackTrace();
	    return ex.toString();
	}
